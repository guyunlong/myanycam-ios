...
//static const CGFloat kPadding = 200;
...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled showLicense:(BOOL)showLicenseButton {
  ...
  if( self ) {
    ...
    CGFloat rectSize = self.frame.size.width - _customkPadding/*kPadding*/ * 2;
    if (!oneDMode) {
      cropRect = CGRectMake(_customkPadding/*kPadding*/, (self.frame.size.height - rectSize) / 2 - 30, rectSize, rectSize);
    } else {
      CGFloat rectSize2 = self.frame.size.height - _customkPadding/*kPadding*/ * 2;
      cropRect = CGRectMake(_customkPadding/*kPadding*/, _customkPadding/*kPadding*/, rectSize, rectSize2);		
    }
...
//        else
//        {
//            CGSize textSize  = [text sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15.0f]}];
//            CGPoint textPoint = CGPointMake(self.bounds.size.height / 2 - textSize.width / 2, self.bounds.size.width * -1.0f + 20.0f);
//            [text drawAtPoint:textPoint withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15.0f]}];
//
//        }
//	}
//	else {
...
//		CGContextMoveToPoint(c, rect.origin.x + offset, rect.origin.y + _customkPadding/*kPadding*/);
//		CGContextAddLineToPoint(c, rect.origin.x + offset, rect.origin.y + rect.size.height - _customkPadding/*kPadding*/);
...
- (void)layoutSubviews {
  ...
  if (cancelButton) {
    if (oneDMode) {
      ...
      CGSize theSize = CGSizeMake(133, 35);
      CGRect rect = self.frame;
      CGRect theRect = CGRectMake((rect.size.width - theSize.width) / 2, cropRect.origin.y + cropRect.size.height + 60, theSize.width, theSize.height);
      [cancelButton setFrame:theRect];
    }
      
      [cancelButton setAutoresizingMask:UIViewAutoresizingFlexibleBottomMargin];
