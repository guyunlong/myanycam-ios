...
#ifdef DEBUG
  DebugLog(@"parsing result:\n<<<\n%@\n>>>\n", s);
#endif
  for (Class c in [self resultParsers]) {
#ifdef DEBUG
    DebugLog(@"trying %@", NSStringFromClass(c));
#endif
    ParsedResult *result = [c parsedResultForString:s format:barcodeFormat];
    if (result != nil) {
#ifdef DEBUG
      DebugLog(@"parsed as %@ %@", NSStringFromClass([result class]), result);
#endif
      return result;
    }
  }
...
#ifdef DEBUG
  DebugLog(@"No result parsers matched. Falling back to text.");
#endif
