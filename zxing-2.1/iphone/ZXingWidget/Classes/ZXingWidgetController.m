...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
  ...
  if (self) {
    ...
    self.showLicense = shouldShowLicense;
      ...
      CGRect rect = CGRectMake(0, 44+20, self.view.bounds.size.width, self.view.bounds.size.height - 64);
      
      if([[UIDevice currentDevice].systemVersion floatValue] >=7.0) {

          self.edgesForExtendedLayout = UIRectEdgeNone;
          rect = CGRectMake(0, 64, self.view.bounds.size.width, self.view.bounds.size.height -64);
      }

    beepSound = -1;
    decoding = NO;
    
    self.view.backgroundColor = [UIColor colorWithRed:35/255.0 green:36/255.0 blue:38/255.0 alpha:1.0];
    OverlayView *theOverLayView = [[OverlayView alloc] initWithFrame:rect
    ...
    [self.overlayView bringSubviewToFront:self.overlayView.cancelButton];
      
      [self.overlayView.cancelButton setBackgroundImage:[UIImage imageNamed:@"login_btn.png"]forState:UIControlStateNormal];
      [self.overlayView.cancelButton setBackgroundImage:[UIImage imageNamed:@"login_btn_hover.png"]forState:UIControlStateHighlighted];
      [self.overlayView.cancelButton setTitle:NSLocalizedString(@"Manually add", nil) forState:UIControlStateNormal];
      
      
      CGRect cropRect = [overlayView cropRect];
      UIImageView * scanImageView = [[UIImageView alloc] initWithFrame:cropRect];
      scanImageView.image = [UIImage imageNamed:@"Scanning.png"];
      [self.overlayView addSubview:scanImageView];
      [scanImageView release];
      
      scanImageView = [[UIImageView alloc] initWithFrame:CGRectMake(42.5, cropRect.origin.y , 235, 4)];
      scanImageView.image = [UIImage imageNamed:@"Scanning1.png"];
      [self.overlayView addSubview:scanImageView];
      self.overlayView.line = scanImageView;
      [scanImageView release];
      
      NSTimer* timer = [NSTimer scheduledTimerWithTimeInterval:0.04 target:self selector:@selector(animation1) userInfo:nil repeats:YES];
      self.scanTimer  = timer;
      
      UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake((self.overlayView.frame.size.width - 300)/2, cropRect.origin.y + cropRect.size.height + 20, 300, 30)];
      label.text = NSLocalizedString(@"Align the QR Code within the frame to scan", nil);
...
-(void)animation1
{
    ...
    if (upOrdown == NO) {
        num ++;
        self.overlayView.line.frame = CGRectMake(42.5, cropRect.origin.y+2*num, 235, 4);
        if (2*num >= 246) {
            upOrdown = YES;
        }
    }
...
- (UIImage *)resizableImage:(UIImage *)orginalImage {
    
    UIImage *scaledImage = [UIImage imageWithCGImage:orginalImage.CGImage scale:2 orientation:UIImageOrientationUp];
...
- (void)cancelled {
  ...
  if (delegate != nil) {
    ...
    [delegate zixngCongrollerManuallyAdd:self];
...
- (void)viewDidLoad{
    ...
    [nav setBackgroundImage: [self resizableImage:[UIImage imageNamed:@"topBar.png"] ]forBarMetrics:UIBarMetricsDefault];
    
    UINavigationItem * navItem = [[UINavigationItem alloc] init];
    UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 100, 20)];
    label.text = NSLocalizedString(@"scan QR code", nil);
    label.textColor = [UIColor whiteColor];
    label.backgroundColor = [UIColor clearColor];
    ...
    //创建一个左边按钮
    UIImage * buttonImage = [UIImage imageNamed:@"icon_Return.png"] ;
    UIImage * buttonImageSelect = [UIImage imageNamed:@"icon_Return_hover.png"];
    UIButton * playNowButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 32, 32)];
    ...
    [playNowButton addTarget:self action:@selector(goBack) forControlEvents:UIControlEventTouchUpInside];

    UIBarButtonItem * finishButton =[[UIBarButtonItem alloc] initWithCustomView:playNowButton];
    navItem.leftBarButtonItem = finishButton;
...
*/
  ...
  // NSLog(@"prev %p %@", self.prevLayer, self.prevLayer);
    CGRect rect =  CGRectMake(0, self.view.bounds.origin.y+ 20 +44, self.view.bounds.size.width, self.view.bounds.size.height-44);

    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
        
        rect = CGRectMake(0, self.view.bounds.origin.y + 44+20, self.view.bounds.size.width, self.view.bounds.size.height-44);

    }
  self.prevLayer.frame = rect;//self.view.bounds;
