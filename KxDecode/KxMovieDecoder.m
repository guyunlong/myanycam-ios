...
static void avStreamFPSTimeBase(AVStream *st, CGFloat defaultTimeBase, CGFloat *pFPS, CGFloat *pTimeBase)
{
    ...
    if (st->codec->ticks_per_frame != 1) {
        DebugLog(@"WARNING: st.codec.ticks_per_frame=%d", st->codec->ticks_per_frame);
...
static NSArray *collectStreams(AVFormatContext *formatCtx, enum FFMAVMediaType codecType)
{
    ...
    NSMutableArray * temp_ma = [ma copy];
    
    return [temp_ma autorelease];
}

//static NSData * copyFrameData(UInt8 *src, int linesize, int width, int height)
//{
//    width = MIN(linesize, width);
////    NSMutableData *md = [NSMutableData dataWithCapacity: width * height];
//    
//    DebugLog(@"width %d",width);
//    NSMutableData *md = [[NSMutableData alloc] initWithCapacity: width * height];
//
...
//       DebugLog(@"md %@  src = %c",md,*src);
//     DebugLog(@"linesize = %d width = %d height = %d",linesize,width,height);
    return [md autorelease];
}
...
//@property (readwrite, nonatomic, retain) NSData *rgb;
//@end
...
- (UIImage *) asImage
{
    ...
    CGDataProviderRef provider = CGDataProviderCreateWithCFData(( CFDataRef)(_rgb));
    if (provider) {
...
@synthesize luma = _luma;
@synthesize chromaB = _chromaB;
@synthesize chromaR = _chromaR;
...
- (UIImage *) asImage
{
    ...
    CGDataProviderRef provider = CGDataProviderCreateWithCFData(( CFDataRef)(_picture));
    if (provider) {
...
- (void)handleVieoFrameWithFrame:(AVFrame *)pavframe andvideoCodecCtx:(AVCodecContext *)videoCodecCtx andKxVideoFrameYUV:(KxVideoFrameYUV *)vFrameYUV{
    if (!pavframe->data[0] || !pavframe->data[1] || !pavframe->data[2])
        return;
    ...
    vFrameYUV.luma = copyFrameData(pavframe->data[0],
                                   pavframe->linesize[0],
                                   videoCodecCtx->width,
                                   videoCodecCtx->height);
    
    vFrameYUV.chromaB = copyFrameData(pavframe->data[1],
                                      pavframe->linesize[1],
                                      videoCodecCtx->width / 2,
                                      videoCodecCtx->height / 2);
    
    vFrameYUV.chromaR = copyFrameData(pavframe->data[2],
                                      pavframe->linesize[2],
                                      videoCodecCtx->width / 2,
                                      videoCodecCtx->height / 2);
