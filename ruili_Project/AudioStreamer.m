...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		{
			DebugLog(@"%@", [AudioStreamer stringForErrorCode:anErrorCode]);
		}
	...
	@synchronized(self)
	{
		if (state != AS_STARTING_FILE_THREAD)
		{
			if (state != AS_STOPPING &&
				state != AS_STOPPED)
			{
				DebugLog(@"### Not starting audio thread. State code is: %ld", (long)state);
			}
...
- (void)startReceiveData{
    ...
    if (!audioFileStream)
    {
        ...
        {
            self.fileExtension = @"aac";//[[url path] pathExtension];
        }
    ...
    while (self.state != AS_STOPPING &&stopReason != AS_STOPPING_USER_ACTION) {
        ...
        if (mediaData) {
            ...
            length = [mediaData.mediaData length];
...
- (void)enqueueBuffer
{
	@synchronized(self)
	{
		...
		if (state == AS_BUFFERING || state == AS_WAITING_FOR_DATA ||
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY))
		{
			...
			//
			if (state == AS_FLUSHING_EOF || buffersUsed == kNumAQBufs_Stream - 1)
			{
		...
		// go to next buffer
		if (++fillBufferIndex >= kNumAQBufs_Stream) fillBufferIndex = 0;
...
- (void)createQueue
{
	...
	for (unsigned int i = 0; i < kNumAQBufs_Stream; ++i)
	{
	...
	packetDescriptions:(AudioStreamPacketDescription *)inPacketDescriptions;
{
	...
	if (inPacketDescriptions)
	{
		for (int i = 0; i < inNumberPackets; ++i)
		{
			...
			size_t packetsDescsRemaining = kAQMaxPacketDescs_Stream - packetsFilled;
...
//
	...
	DebugLog(@"Queued buffers: %d", buffersUsed);
#endif
...
//				[NSRunLoop currentRunLoop];
			...
			{
				DebugLog(@"AudioQueue changed state in unexpected way.");
			}
		}
	}
