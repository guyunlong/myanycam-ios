...
static int refresh_thread(void *opaque)
{
    ...
    while ((rtspPlayViewController.videoPlayState == VideoPlayStatePlaying)&& !is->abort_request) {
//        SDL_Event event;
...
static RTSPPlayViewController * rtspPlayViewController = nil;
        ...
        display:
            ...
            if (!display_disable)
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (rtspPlayViewController) {
                        
                        [rtspPlayViewController showVideo];
                    }
...
//初始化图片  //!!!分配 SDL 库需要的 Overlay 显示表面,并设置长宽属性。
...
//    if (!vp->bmp || vp->reallocate || !vp->allocated ||
    if (!isFrameOk || vp->reallocate || !vp->allocated ||
        ...
        vp->width = src_frame->width;
        vp->height = src_frame->height;
        is->videoFrame = src_frame;//解码后视频帧
...
static int get_video_frame(VideoState *is, AVFrame *frame, int64_t *pts, AVPacket *pkt)
{
    ...
    if(avcodec_decode_video2(is->video_st->codec, frame, &got_picture, pkt) < 0)//把包转换为帧
    {
...
static int video_thread(void *arg)
{
    AVPacket pkt = { 0 };
    VideoState *is = arg;
    AVFrame *frame = avcodec_alloc_frame();//分配解码帧缓存,保存帧数据
    ...
    for (;;) {
        ...
        ret = get_video_frame(is, frame, &pts_int, &pkt); //从队列中取数据帧/数据包 视频帧
        
//        if (is->video_st->codec->width * is->video_st->codec->height >= HDWIDTHHEIGHTTOTAL) {
//            
//            [rtspPlayViewController stopWithError:VideoPlayErrorTypeHDError andError:nil];
//            return 0;
//        }
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    wanted_spec.channels = av_get_channel_layout_nb_channels(wanted_channel_layout);//通道数
    wanted_spec.freq = wanted_sample_rate;//采样率
    if (wanted_spec.freq <= 0 || wanted_spec.channels <= 0) {
        fprintf(stderr, "Invalid sample rate or channel count!\n");
        return -1;
    }
    wanted_spec.format = AUDIO_S16SYS;//S16SYS”中的 S 表示有符号的 signed,16 表示每个样本是 16 位长的,SYS 表示大小头的顺序是与使用的系统 相同的
    wanted_spec.silence = 0;//这是用来表示静音的值。因为声音采样是有符号的,所以 0 当然就 是这个值。
    wanted_spec.samples = SDL_AUDIO_BUFFER_SIZE;// 这是当我们想要更多声音的时候,我们想让 SDL 给出来的声音缓冲 区的尺寸。一个比较合适的值在 512 到 8192 之间;ffplay 使用 1024。
    wanted_spec.callback = sdl_audio_callback;//回调函数
    wanted_spec.userdata = opaque;//这个是 SDL 供给回调函数运行的参数。我们将让回调函数得到整 个编解码的上下文
    while (SDL_OpenAudio(&wanted_spec, &spec) < 0)//打开声音
    {
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    AVCodecContext *avctx;//编解码器上下文,编解码信息
    AVCodec *codec;//真正的编解码器
    AVDictionary *opts;
    ...
    switch(avctx->codec_type){
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if(video_codec_name) codec= avcodec_find_decoder_by_name(video_codec_name);
    ...
    switch (avctx->codec_type) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            DebugLog(@"line:%d width:%d  height:%d", __LINE__, is->video_st->codec->width, is->video_st->codec->height);

            packet_queue_start(&is->videoq);
            is->video_tid = SDL_CreateThread(video_thread,"video_tid",  is);//创建视频线程
            break;
        case AVMEDIA_TYPE_SUBTITLE:
...
{
    VideoState *is = arg;
    AVFormatContext *ic = NULL;//存储媒体格式信息
    int err, i, ret;
...
//    Open an input stream and read the header. The codecs are not opened.
    err = avformat_open_input(&ic, is->filename, is->iformat, &format_opts);//这个函数读取文件的头部并且把信息保存到 我们给的 AVFormatContext ic 结构体中
    if (err < 0) {
    ...
    err = avformat_find_stream_info(ic, opts);//检查在文件中的流的信息,这个函数为 pFormatCtx->streams 填充上正确的信息
    ...
    //如果有音频流,就调用函数打开音频解码器 并 启动 音频 解码线程。
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0) {
        stream_component_open(is, st_index[AVMEDIA_TYPE_AUDIO]);
    }

    ret = -1;
    //如果有视频流,就调用函数打开视频解码器 并 启动 视频 解码线程。
    ...
    for (;;) {
        ...
        if (is->paused &&
            (!strcmp(ic->iformat->name, "rtsp") ||
             (ic->pb && !strncmp(input_filename, "mmsh:", 5)))) {
                ...
                /* XXX: horrible */
                SDL_Delay(10);//如果队列满,就稍微延时一下 // if the queue are full, no need to read more,wait 10 ms
                continue;
            }
#endif
        if (is->seek_req) {
        ...
        ret = av_read_frame(ic, pkt);//读取一个包并且把它保存到 AVPacket 结构体中
        if (ret < 0) {
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    is = av_mallocz(sizeof(VideoState));//分配内存
    if (!is)
        return NULL;
    av_strlcpy(is->filename, filename, sizeof(is->filename));//拷贝文件名
    is->iformat = iformat;
    is->ytop    = 0;
    is->xleft   = 0;
    
    /* start video display */
    is->pictq_mutex = SDL_CreateMutex();// 互斥锁
    is->pictq_cond  = SDL_CreateCond();
    
    is->subpq_mutex = SDL_CreateMutex();
    is->subpq_cond  = SDL_CreateCond();
    
    packet_queue_init(&is->videoq);//视频队列初始化
    packet_queue_init(&is->audioq);//音频队列初始化
    packet_queue_init(&is->subtitleq);//字母队列初始化
    
    is->continue_read_thread = SDL_CreateCond();//条件变量
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(netErrorNotification)
                                                 name:KNotificationMYNetAsynsocketError
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(stopOpenGlRender)
                                                 name:KNotificationResignActive
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(restartOpenGlRender)
                                                 name:KNotificationBecomeActive
                                               object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(manualRecordRespone:)
                                                 name:KNotificationAPmodeManualRecord
                                               object:nil];
    ...
    if ([ToolClass systemVersionFloat] >= 7.0) {
        ...
        CGRect rect = self.topBarNavigationbar.frame;
        rect.size.height = 64;
        self.topBarNavigationbar.frame = rect;
        
    }
    
    [self.topBarNavigationbar setBackgroundImage:[[UIImage imageNamed:@"topBar.png"] resizableImage] forBarMetrics:UIBarMetricsDefault];
    
//    NSString * imageNormalStr = @"backButtonNormal.png";
//    NSString * imageSelectStr = @"backButtonSelect.png";
//    UIBarButtonItem *backButton = [ViewToolClass customBarButtonItem:imageNormalStr buttonSelectImage:imageSelectStr title:NSLocalizedString(@"Back", nil) size:CGSizeMake(74, 30) target:self  action:@selector(goToBack)];
//    self.topBarNavigationbarItem.leftBarButtonItem = backButton;
    
    rtspPlayViewController = self;
    ...
    [self.view insertSubview:frameView belowSubview:self.videoControlButton];
...
- (void)removeWaitView{
    ...
    if (!self.playBackView.hidden ) {
        
        [self.playBackView customHideWithAnimation:YES duration:0.2];
        [self.playCircleImageView removeAnimations];
    }
...
- (void)whirlVplay{
    ...
    [self.playCircleImageView addAnimation:spinAnim forKey:@"playWait"];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MobClick beginLogPageView:@"RTSPPlayViewController"];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [MobClick endLogPageView:@"RTSPPlayViewController"];

}

- (void)viewDidUnload
{
    [self setVideoControlButton:nil];
    [self setTopBarNavigationbar:nil];
    [self setTopBarNavigationbarItem:nil];
    [self setPlayCircleImageView:nil];
    [self setPlayBackView:nil];
    [self setPlaytipLabel:nil];
...
- (BOOL)shouldAutorotate
{
    ...
    switch (orientation ) {
        case UIInterfaceOrientationPortrait:
        {
            ...
            [self.topBarNavigationbar setHidden:NO];
        ...
        case UIInterfaceOrientationLandscapeRight:
        {
            ...
            [self.topBarNavigationbar setHidden:YES];
...
- (void)startPlayWithURLString:(NSString *)playURLString {
    ...
    cmd=[dealWithCommend addElement:cmd eName:@"cmd" eData:@"WATCH_CAMERA"];
    cmd=[dealWithCommend addElement:cmd eName:@"fromuid" eData:@"0"];
    cmd=[dealWithCommend addElement:cmd eName:@"touid" eData:@"0"];
    cmd=[dealWithCommend addElement:cmd eName:@"videosize" eData:[NSString stringWithFormat:@"%d",VIDEO_SIZE_TYPE_480P]];
    [[[AppDelegate getAppDelegate] mygcdSocketEngine] writeDataOnMainThread:cmd tag:0  waitView:NO];
    ...
    av_register_all();//注册了所有的文件格式和编解码器的库
    avformat_network_init();
        
    if (!input_filename) {
        //路径错误
        [rtspPlayViewController stopWithError:VideoPlayErrorTypeInput andError:nil];
    }
...
//        fprintf(stderr, "(Did you set the DISPLAY variable?)\n");
        //exit(1);
        //SDL错误
        [rtspPlayViewController stopWithError:VideoPlayErrorTypeSDLError andError:nil];
    ...
    if (!is) {
        ...
        //do_exit(NULL);
        [rtspPlayViewController stopWithError:VideoPlayErrorTypeInitError andError:nil];
...
- (void)showVideo {
    ...
    if (isFrameOk) {
        ...
        if (kxVideoFrame.chromaB && kxVideoFrame.chromaR && kxVideoFrame.luma && _decodeFlag) {
...
- (void)stopWithError:(VideoPlayErrorType)errotType andError:(NSError *)error {
    ...
    //然后说明原因
    DebugLog(@"stop error:%@", [error description]);
...
- (IBAction)playAction:(id)sender {
    ...
    [self startPlayWithURLString:KRTSP_URL];
    _decodeFlag = YES;
...
- (void)sendStopWatchCamera{
    ...
    NSString * cmd = [BuildSocketData buildStopWatchCamera:0
                                                  cameraid:@""
                                                  password:@""];
...
- (void)dealloc {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:KNotificationAPmodeManualTakePhoto
                                                  object:nil];
    rtspPlayViewController  = nil;
    [_videoControlButton release];
    [_topBarNavigationbar release];
    [_topBarNavigationbarItem release];
    [_playCircleImageView release];
    [_playBackView release];
    [_playtipLabel release];
    [_muteButton release];
...
- (IBAction)muteButtonAction:(id)sender {
    ...
    NSString * pressStr = @"正在监听.png";
    NSString *  norStr = @"未在监听.png";
    ...
    [button setButtonBgImage:imageView highlight:imageView];
...
- (IBAction)takePhotoAction:(id)sender {
    ...
    [[AppDelegate getAppDelegate].mygcdSocketEngine sendTakePhoto:0 cameraid:0];
...
- (IBAction)recordButtonAction:(id)sender {
    ...
    if (_flagStartRecord) {
        ...
        [[AppDelegate getAppDelegate].mygcdSocketEngine sendManualRecordWithSwith:[MYDataManager shareManager].userInfoData.userId cameraid:0 swithFlag:0];
    }
    else
    {
        _flagStartRecord = YES;
        [[AppDelegate getAppDelegate].mygcdSocketEngine sendManualRecordWithSwith:[MYDataManager shareManager].userInfoData.userId cameraid:0 swithFlag:1];
        imageView = [UIImage imageNamed:pressStr];
