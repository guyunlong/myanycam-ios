...
- (AsyncImageLoader *)init
{
	if ((self = [super init]))
	{
        self.cache = [[[self class] defaultCache] autorelease];
        _concurrentLoads = 2;
...
- (NSURL *)URLForTarget:(id)target action:(SEL)action
{
    ...
    for (int i = (int)[_connections count] - 1; i >= 0; i--)
    {
        ...
        if (connection.target == target && connection.success == action)
        {
            return [[connection.URL retain] autorelease];
        }
    }
    return nil;
}
...
{
	[[AsyncImageLoader sharedLoader] loadImageWithURL:imageURL target:self action:@selector(setBorderImage:)];
}

- (NSURL *)imageURL
{
	return [[AsyncImageLoader sharedLoader] URLForTarget:self action:@selector(setBorderImage:)];
}
...
- (void)setBorderImage:(UIImage *)image{
    ...
    [self setImage:image borderWidth:5.0 shadowDepth:10.0 controlPointXOffset:30.0 controlPointYOffset:70.0];
}
