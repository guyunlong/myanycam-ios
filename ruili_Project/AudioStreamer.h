...
#define kNumAQBufs_Stream 16			// Number of audio queue buffers we allocate.
...
#define kAQMaxPacketDescs_Stream 6	// Number of packet descriptions in our array
...
@interface AudioStreamer : NSObject
{
	...
	AudioQueueBufferRef audioQueueBuffer[kNumAQBufs_Stream];		// audio queue buffers
	AudioStreamPacketDescription packetDescs[kAQMaxPacketDescs_Stream];	// packet descriptions for enqueuing audio
	unsigned int fillBufferIndex;	// the index of the audioQueueBuffer that is being filled
	UInt32 packetBufferSize;
	size_t bytesFilled;				// how many bytes have been filled
	size_t packetsFilled;			// how many packets have been filled
	bool inuse[kNumAQBufs_Stream];			// flags to indicate that a buffer is still in use
...
@property (retain, nonatomic) MediaData * selectData;
