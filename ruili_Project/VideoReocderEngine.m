...
//            totalsize = nLen+8+[_spspps getPpsSize]+[_spspps getPpsSize];
//            tmp = (uint8_t*)realloc(tmp, totalsize);
//            memcpy(tmp, nalu_header, 4);
//            memcpy(tmp+4, [_spspps getSps], [_spspps getSpsSize]);
//            memcpy(tmp+4+[_spspps getSpsSize], nalu_header, 4);
//             memcpy(tmp+4+[_spspps getSpsSize]+4, [_spspps getPps], [_spspps getPpsSize]);
            pkt.stream_index = video_st->index;
            pkt.data = (uint8_t *)[_spspps getIdr];
            pkt.size = [_spspps getIdrLength];
      ...
      //  v_time=++v_flag==1?av_gettime():v_time;//取得第一次保存的时间
        ...
        static int ptsInc = 0;
        pkt.pts = (ptsInc++) * (30000/STREAM_FRAME_RATE);
        pkt.dts=pkt.pts;
