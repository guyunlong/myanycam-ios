...
- (BOOL)createSocket4:(BOOL)useIPv4 socket6:(BOOL)useIPv6 error:(NSError **)errPtr
{
	...
	int(^createSocket)(int) = ^int (int domain) {
        ...
        status =  setsockopt(socketFD,SOL_SOCKET,SO_RCVBUF,(const char*)&nRecvBuf,sizeof(int));
		if (status == -1)
		{
			if (errPtr)
				*errPtr = [self errnoErrorWithReason:@"Error disabling sigpipe (setsockopt)"];
        ...
        status = setsockopt(socketFD,SOL_SOCKET,SO_SNDBUF,(const char*)&nSendBuf,sizeof(int));
        if (status == -1)
		{
			if (errPtr)
				*errPtr = [self errnoErrorWithReason:@" SO_SNDBUF Error disabling sigpipe (setsockopt)"];
...
//            char*p = inet_ntoa(sockaddr4.sin_addr);
//            printf("ip addr is %s\n",p);
//            printf("ip port is %d\n",sockaddr4.sin_port);
