...
void show_help_children(const AVClass *andidaclass, int flags)
{
    const AVClass *child = NULL;
    if (andidaclass->option) {
        av_opt_show2(&andidaclass, NULL, flags, 0);
        printf("\n");
    }

    while ((child = av_opt_child_class_next(andidaclass, child)))
                                     ...
                                     struct tm *tm)
{
    ...
    while ((c = *(template++))) {
        if (c == '%') {
            ...
            switch (c) {
            case 'p':
//                av_bprintf(bp, "%s", program_name);andida
                break;
            case 't':
...
//    const char *indent = flags & INDENT? "  " : "";
//
////    av_log(NULL, level, "%s version " FFMPEG_VERSION, program_name);andida
//    if (flags & SHOW_COPYRIGHT)
////        av_log(NULL, level, " Copyright (c) %d-%d the FFmpeg developers",
////               program_birth_year, this_year);  andida
//    av_log(NULL, level, "\n");
...
int codec_get_buffer(AVCodecContext *s, AVFrame *frame)
{
    ...
    *pool            = buf->next;
    ...
    frame->opaque        = buf;
//    frame->type          = FF_BUFFER_TYPE_USER; andida
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(buf->data); i++) {
//        frame->base[i]     = buf->base[i];  // XXX h264.c uses base though it shouldn't andida
