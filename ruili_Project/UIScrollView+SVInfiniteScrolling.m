...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        CGFloat scrollOffsetThreshold = scrollViewContentHeight-self.scrollView.bounds.size.height + 100;
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        if (![self isDescendantOfView:customView] ) {
        ...
        if ([customView isKindOfClass:[WaitLogoAnimationView class]]) {
            
            WaitLogoAnimationView * view = (WaitLogoAnimationView *)customView;
            
            switch (newState) {
                case SVInfiniteScrollingStateStopped:{
                    
                    [view stopRotateAnimation];
                }
                    break;
                    
                case SVInfiniteScrollingStateTriggered:{
                    
                    DebugLog(@"Release to refresh");
                    [view showTipLable:self.tipStr];
                }
                    break;
                    
                case SVInfiniteScrollingStateLoading:{
                    
                    [view showTipLable:NSLocalizedString(@"Loading...",nil)];
                    [view startRotateAnimating];
                }
                    break;
            }
        }
